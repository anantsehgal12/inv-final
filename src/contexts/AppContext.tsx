...
export function AppProvider({ children }: AppProviderProps) {
  ...
  const computeNextInvoiceNumber = useCallback((type: 'invoice' | 'proforma' = 'invoice') => {
    const year = new Date().getFullYear();
    const yearInvoices = invoices.filter(i => i.type === type && i.invoiceDate.getFullYear() === year);
    const nextNumber = Math.max(yearInvoices.length + 1, settings[type].startingNumber);
    const prefix = settings[type].numberPrefix;
    const suffix = settings[type].numberSuffix;
    let invoiceNumber = `${prefix}${nextNumber.toString().padStart(2, '0')}`;
